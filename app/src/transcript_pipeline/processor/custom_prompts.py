...
def create_master_document_for_expansion_prompt(transcript_text, target_length, chapter_number, segment_text, master_document=None):
    ...
    Supports both recreation mode (create new content) and expansion mode (expand existing content).
    ...
    # Check if this is recreation mode by looking for recreation mode indicators
    recreation_indicators = ["NEW TOPICS FOR CHAPTER", "RECREATION MODE", "IGNORE ORIGINAL CONTENT", "CREATE ENTIRELY NEW"]
    is_recreation_mode = master_document and any(indicator in master_document for indicator in recreation_indicators)
    
    # Additional logging for debugging
    if master_document:
        if is_recreation_mode:
            print(f"🎯 RECREATION MODE detected in custom_prompts.py for Chapter {chapter_number}")
        else:
            print(f"📄 EXPANSION MODE detected in custom_prompts.py for Chapter {chapter_number}")
            print(f"   Master document preview: {master_document[:200]}...")
    
    if is_recreation_mode:
        # RECREATION MODE: Create entirely new content based on instructions
        return f"""
# CHAPTER CREATION DIRECTIVE (RECREATION MODE)

## CRITICAL INSTRUCTION: RECREATION MODE ACTIVE
You are in RECREATION MODE. This means you should CREATE ENTIRELY NEW CONTENT based on your role and the script structure provided.
DO NOT expand on the original transcript segment. Instead, create new content that follows your instructions.

## MASTER DOCUMENT OUTLINE
{master_doc_extract if master_doc_extract else "Create an engaging and detailed chapter based on your role and script structure."}
...
- MODE: RECREATION (create new content, ignore original transcript)
- You are creating Chapter {chapter_number} ONLY in this response

## RECREATION MODE INSTRUCTIONS
Since you are in RECREATION MODE:
1. IGNORE the original transcript segment provided
2. CREATE entirely new content based on your role and script structure
3. Follow the topics outlined in the master document above
4. The content should advance the narrative defined in your role
5. This chapter should fit logically into the overall structure provided
6. Focus on creating engaging, original content that serves your purpose

## YOUR TASK
1. Create "Chapter {chapter_number}" followed by an engaging title that fits your role
2. CREATE new content (not expand existing) following the master document outline
...
5. Ensure the content aligns with your role and script structure
...
4. If too long: Remove less essential details while preserving core content
...
- YOU ARE IN RECREATION MODE - CREATE NEW CONTENT, DO NOT EXPAND EXISTING CONTENT
"""
    else:
        # EXPANSION MODE: Expand existing content
        return f"""
